...
while True:
    ...
    #lặp qua từng layerOutputs
    for output in layerOutputs:
        # lặp qua từng object được detect
        for detection in output:
            ...
            # bỏ qua các detect không tốt
            if confidence > args["confidence"]:
                # đưa tỉ lệ bbox phù hợp kích thước ảnh
                # YOLO trả về giá trị center (x, y) và width, height
                box = detection[0:4]*np.array([W, H, W, H])
                (centerX, centerY, width, height) = box.astype("int")
                #tính các giá trị x, y là góc trên của bbox
                x = int(centerX - (width/2))
                y = int(centerY - (height/2))
                # cập nhật bboxes, confidences, classIDs
    ...
    # đảm bảo có ít nhất 1 detection tồn tại
    if len(idxs) > 0:
